...
impl GeoStore {
    ...
    pub async fn init(&self) -> Result<(), JsValue> {
        ...
        let _rexie = Rexie::builder(&self.db_name)
            .version(1)
            .add_object_store(
                ObjectStore::new("features")
                    .key_path("id")
                    .auto_increment(false),
            )
            .add_object_store(
                ObjectStore::new("factors")
                    .key_path("id")
                    .auto_increment(false),
            )
            .add_object_store(
                ObjectStore::new("results")
                    .key_path("id")
                    .auto_increment(false),
            )
            .add_object_store(ObjectStore::new("crs_defs").key_path("epsg"))
            .build()
            .await
    ...
    pub async fn put_feature(&self, id: &str, geojson_feature: &str) -> Result<(), JsValue> {
        ...
        let rexie = rexie::Rexie::builder(&self.db_name)
            .build()
            .await
            .map_err(|e| JsValue::from_str(&format!("DB open: {e}")))?;

        let tx = rexie
            .transaction(&["features"], rexie::TransactionMode::ReadWrite)
    ...
    pub async fn get_feature(&self, id: &str) -> Result<JsValue, JsValue> {
        ...
        let tx = rexie
            .transaction(&["features"], rexie::TransactionMode::ReadOnly)
            .map_err(|e| JsValue::from_str(&format!("Transaction: {e}")))?;
    ...
    pub async fn get_all_features(&self) -> Result<String, JsValue> {
        ...
        let features: Vec<serde_json::Value> = serde_wasm_bindgen::from_value(JsValue::from(all))
