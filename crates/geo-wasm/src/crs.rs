...
use geo_core::crs::{builtin, CrsRegistry};
...
impl CrsEngine {
    fn get_crs_inner(&self, epsg: u16) -> GeoResult<serde_json::Value> {
        match self.registry.get(epsg) {
            Some(def) => serde_json::to_value(&CrsDefJs {
                epsg: def.epsg,
                name: def.name.to_string(),
                category: format!("{:?}", def.category),
            })
            .map_err(geo_core::errors::GeoError::Serde),
            None => Err(geo_core::errors::GeoError::Validation(format!(
                "EPSG:{epsg} not found"
            ))),
        }
    }
    ...
    fn transform_inner(
        ...
        match (from_epsg, to_epsg) {
            (4326, 3857) => {
                let (mx, my) = builtin::wgs84_to_mercator(x, y);
                return Ok(Box::new([mx, my]));
            }
            (3857, 4326) => {
                let (lx, ly) = builtin::mercator_to_wgs84(x, y);
                return Ok(Box::new([lx, ly]));
            }
            (4326, 9000) => {
                let (gx, gy) = builtin::wgs84_to_gcj02(x, y);
                return Ok(Box::new([gx, gy]));
            }
            (9000, 4326) => {
                let (wx, wy) = builtin::gcj02_to_wgs84(x, y);
                return Ok(Box::new([wx, wy]));
            }
            (9000, 9001) => {
                let (bx, by) = builtin::gcj02_to_bd09(x, y);
            ...
            (4326, 3405) => {
                let (ex, ey) = builtin::wgs84_to_equal_area(x, y);
                return Ok(Box::new([ex, ey]));
            }
            _ => {}
        }

        Err(geo_core::errors::GeoError::Validation(format!(
    ...
    fn transform_batch_inner(
        ...
        if !coords.len().is_multiple_of(2) {
            return Err(geo_core::errors::GeoError::Validation(
                "coords length must be even".into(),
            ));
        }

        let mut result = Vec::with_capacity(coords.len());
        for chunk in coords.chunks_exact(2) {
            let arr = self.transform_inner(from_epsg, to_epsg, chunk[0], chunk[1])?;
            result.push(arr[0]);
            result.push(arr[1]);
        }
...
// ── WASM bindings (JS-facing thin wrappers) ──────────────────────
...
impl CrsEngine {
    ...
    pub fn get_crs(&self, epsg: u16) -> JsValue {
        match self.get_crs_inner(epsg) {
            Ok(val) => serde_wasm_bindgen::to_value(&val).unwrap_or(JsValue::NULL),
            Err(_) => JsValue::NULL,
        }
    }
    ...
    pub fn transform(
        ...
        self.transform_inner(from_epsg, to_epsg, x, y)
    ...
    pub fn transform_batch(
        ...
        self.transform_batch_inner(from_epsg, to_epsg, coords)
            .map_err(|e| JsValue::from_str(&e.to_string()))
    }
...
mod tests {
    ...
    fn test_transform_4326_to_3857() {
        ...
        let result = engine.transform_inner(4326, 3857, 104.06, 30.57).unwrap();
    ...
    fn test_transform_wgs84_to_gcj02() {
        let engine = CrsEngine::new();
        let result = engine.transform_inner(4326, 9000, 104.06, 30.57).unwrap();
    ...
    fn test_transform_wgs84_to_bd09() {
        let engine = CrsEngine::new();
        let result = engine.transform_inner(4326, 9001, 104.06, 30.57).unwrap();
        assert!((result[0] - 104.06).abs() > 0.001);
    }
    ...
    fn test_identity() {
        let engine = CrsEngine::new();
        let result = engine.transform_inner(4326, 4326, 113.9, 22.5).unwrap();
