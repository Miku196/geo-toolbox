...
async fn handle_gee(action: GeeAction) -> Result<(), Box<dyn std::error::Error>> {
    match action {
        GeeAction::Classify { aoi, year, output_gcs, params } => {
            ...
            let cid = dispatcher.dispatch_classification(&aoi, year, &output_gcs, extra).await?;
        ...
        GeeAction::Ndvi { aoi, year, output_gcs } => {
            ...
            let mq = create_mq().await?;
            let dispatcher = GeeDispatcher::new(mq);
            let cid = dispatcher.dispatch_ndvi_timeseries(&aoi, year, &output_gcs).await?;
        ...
        GeeAction::Change { aoi, from, to, output_gcs } => {
            ...
            let mq = create_mq().await?;
            let dispatcher = GeeDispatcher::new(mq);
            let cid = dispatcher.dispatch_change_detection(&aoi, from, to, &output_gcs).await?;
...
async fn handle_gdal(action: GdalAction) -> Result<(), Box<dyn std::error::Error>> {
    match action {
        GdalAction::Cog { input, output, compression } => {
            ...
            let opts = CogOptions { compression, ..CogOptions::default() };
        ...
        GdalAction::Ogr2Ogr { input, output, epsg, r#where, overwrite } => {
            ...
            let opts = Ogr2OgrOptions { target_epsg: epsg, where_clause: r#where, overwrite, ..Default::default() };
        ...
        GdalAction::GcsBridge { gcs_uri, prefix, cog, local } => {
            ...
            if local { config.minio_endpoint = None; config.minio_bucket = None; }
...
async fn handle_qgis(action: QgisAction) -> Result<(), Box<dyn std::error::Error>> {
    match action {
        QgisAction::Submit { algorithm, params, input, output, server } => {
            ...
            let job = QgisJob {
                model: None,
                tools: vec![QgisToolStep { algorithm: algorithm.clone(), params: params_value, label: Some(algorithm.clone()) }],
                inputs: vec![QgisInput { name: "input_layer".into(), path: input.clone(), crs: None }],
                output_format: "gpkg".into(),
            };
            let job_id = client.submit(&job).await?;
            println!("Job submitted: {job_id}");
            let result = client.wait_for_job(&job_id, std::time::Duration::from_secs(2)).await?;
        ...
        QgisAction::Batch { algorithm, input, output, extra } => {
            ...
            for pair in &extra_pairs { if pair.len() == 2 { params.push((pair[0].clone(), pair[1].clone())); } }
            params.push(("OUTPUT".into(), output.clone()));
            let result = runner.run_tool(&QgisTool { algorithm: algorithm.clone(), params }).await?;
