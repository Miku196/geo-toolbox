...
pub fn handle(
    ...
    match action {
        CrsAction::List => {
            ...
            for crs in list {
                println!(
                    "{:<6} {:<10} {:<30} {}",
                    crs["epsg"],
                    crs["category"].as_str().unwrap_or(""),
                    crs["name"].as_str().unwrap_or(""),
                    crs["proj4"].as_str().unwrap_or("")
                );
            }
        ...
        CrsAction::Show { epsg } => {
            ...
            match list
                .iter()
                .find(|c| c["epsg"].as_u64() == Some(epsg as u64))
            {
                Some(c) => {
                    println!("EPSG:     {}", c["epsg"]);
                    println!("Name:     {}", c["name"]);
            ...
            if batch {
                ...
                for line in stdin.lock().lines() {
                    ...
                    if let Some((sx, sy)) = line.split_once(',') {
                        ...
                        let result = registry.dispatch_sync(
                            "crs_transform",
                        ...
                        println!(
                            "{},{}",
                            out[0].as_f64().unwrap_or(0.0),
                            out[1].as_f64().unwrap_or(0.0)
                        );
                    }
                }
            } else {
        ...
        CrsAction::Register {
            ...
            println!(
                "CRS registration not persisted. EPSG:{epsg} \"{name}\" added to runtime only."
            );
        }
    }
    Ok(())
