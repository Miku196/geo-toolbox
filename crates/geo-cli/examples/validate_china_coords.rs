...
use geo_core::types::{point, validate_coord, BBox};
use serde::Serialize;
...
fn main() {
    ...
    println!(
        "  BBox::contains(121.5, 31.2) = {} (上海)",
        china_bbox.contains(121.4737, 31.2304)
    );
    println!(
        "  BBox::contains(87.6, 43.8)  = {} (乌鲁木齐)",
        china_bbox.contains(87.6168, 43.8256)
    );
    println!(
        "  BBox::contains(0.0, 0.0)    = {} (不在中国)",
    ...
    match point(116.4074, 39.9042) {
        Ok(p) => println!(
            "  point(116.4074, 39.9042) = Point({}, {}) ✅ (北京)",
            p.x(),
            p.y()
        ),
        Err(e) => println!("  ❌ {}", e),
