...
impl ReportGenerator {
    ...
    /// Render a carbon accounting report to PDF.
    pub fn carbon_report_pdf(&self, data: &CarbonReportData, path: &str) -> GeoResult<()> {
        let (doc, page_idx, layer_idx) =
            PdfDocument::new(&data.title, Mm(210.0), Mm(297.0), "Layer 1");
        let font = doc
            .add_builtin_font(BuiltinFont::Helvetica)
            .map_err(|e| GeoError::Other(format!("printpdf font: {e}")))?;

        let page = doc.get_page(page_idx);
        let layer = page.get_layer(layer_idx);

        let mut y = 280.0; // start from top, step down 5mm per line
        let left = Mm(10.0);
        let font_size = 10.0;
        let line_height = 5.0;
        ...
        macro_rules! write_line {
            ($text:expr, $size:expr) => {
                layer.use_text($text, $size, left, Mm(y), &font);
                y -= $size * 0.4;
            };
        }

        write_line!(&data.title, 16.0);
        y -= 4.0;
        write_line!(
            &format!("AOI: {}   Year: {}", data.aoi_name, data.year),
            font_size
        );
        write_line!(&format!("Source: {}", data.source), font_size);
        write_line!(&format!("Generated: {}", data.generated_at), font_size);
        y -= 6.0;

        write_line!("Summary", 14.0);
        write_line!(
            &format!("Total Emissions: {:.1} tCO₂e", data.total_tco2e),
            12.0
        );
        y -= 4.0;

        write_line!(
            &format!("Landcover Classes: {}", data.breakdown.len()),
            font_size
        );
        y -= 4.0;

        write_line!("Breakdown by Class", 12.0);
        for item in &data.breakdown {
            write_line!(
                &format!(
                    "  {} — {:.1} ha × {:.2} = {:.1} tCO₂e",
                    item.class, item.area_ha, item.factor, item.tco2e
                ),
                font_size
            );
        }
        ...
        write_line!("Audit Trail", 12.0);
        for audit in &data.audit_trails {
            let status = if audit.complete { "OK" } else { "INCOMPLETE" };
            write_line!(
                &format!("  {} — {} — {}", audit.class, audit.factor_hash, status),
                font_size
            );
        }
        y -= 4.0;

        write_line!("Generated by geo-toolbox v0.1.0", 8.0);

        let file = File::create(path).map_err(|e| GeoError::Io(e))?;
        let buf = BufWriter::new(file);
        doc.save(buf)
            .map_err(|e| GeoError::Other(format!("printpdf save: {e}")))?;
        Ok(())
    }
