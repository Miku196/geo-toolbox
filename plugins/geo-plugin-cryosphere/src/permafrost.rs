...
/// Stefan 解: 活动层厚度 = K × √(I)
pub fn active_layer_thickness_stefan(thawing_degree_days: f64, thermal_conductivity: f64, ice_content: f64) -> f64 {
    if thawing_degree_days <= 0.0 { return 0.0; }
...
pub fn freeze_thaw_index(daily_temp_c: &[f64]) -> FrostIndex {
    let fi: f64 = daily_temp_c.iter().map(|&t| if t < 0.0 { -t } else { 0.0 }).sum();
    let ti: f64 = daily_temp_c.iter().map(|&t| if t > 0.0 { t } else { 0.0 }).sum();
    FrostIndex { freezing_index: fi, thawing_index: ti, active_layer_thickness_cm: 0.0 }
}
