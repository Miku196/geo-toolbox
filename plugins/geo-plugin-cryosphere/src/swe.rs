...
pub fn snowmelt_degree_day(temp_c: &[f64], dd_factor_mm_c_day: f64) -> Vec<f64> {
    temp_c.iter().map(|&t| {
        if t > 0.0 { t * dd_factor_mm_c_day } else { 0.0 }
    }).collect()
}

/// 简化解能量平衡融雪
pub fn snowmelt_energy_balance(sw_net: &[f64], lw_net: &[f64], sensible: &[f64], latent: &[f64]) -> Vec<f64> {
    let lf = 334.0; // latent heat of fusion kJ/kg
    sw_net.iter().zip(lw_net.iter()).zip(sensible.iter()).zip(latent.iter()).map(|(((sw, lw), sh), lh)| {
        let total_w_m2 = sw + lw + sh + lh;
        if total_w_m2 > 0.0 { total_w_m2 / lf * 3600.0 * 24.0 / 1000.0 } else { 0.0 }
    }).collect()
}
...
pub fn swe_accumulation(precip_mm: &[f64], temp_c: &[f64], rain_snow_threshold: f64) -> Vec<f64> {
    let mut swe = 0.0;
    precip_mm.iter().zip(temp_c.iter()).map(|(&p, &t)| {
...
/// 完整 SWE + 融雪模拟
pub fn simulate_swe(precip_mm: &[f64], temp_c: &[f64], dd_factor_mm_c_day: f64, rain_snow_c: f64) -> SweResult {
    ...
    SweResult { swe_mm: swe, melt_mm: melt, total_melt_mm: total_melt, max_swe_mm: max_swe, melt_days }
}
