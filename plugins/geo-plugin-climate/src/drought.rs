...
pub fn compute_spei(precip_monthly: &[f64], temp_monthly: &[f64], scale_months: usize) -> Option<SpiResult> {
    ...
    let std: f64 = (acc.iter().map(|&x| (x - mean).powi(2)).sum::<f64>() / acc.len() as f64).sqrt().max(1e-10);
    let mut values = Vec::with_capacity(acc.len());
    for &x in &acc {
...
pub fn compute_pdsi(temp_monthly: &[f64], precip_monthly: &[f64], lat: f64, awc_mm: f64) -> Option<Vec<f64>> {
...
fn gamma_upper_cf(a: f64, z: f64) -> f64 {
    ...
    let mut h = d;
    for i in 1..200 {
        let an = if i % 2 == 1 { (i as f64 + 1.0) / 2.0 - a } else { i as f64 / 2.0 * z };
