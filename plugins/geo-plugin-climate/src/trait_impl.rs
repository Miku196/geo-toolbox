...
use crate::{gcm, idf, drought, kriging};
...
impl ProcessPlugin for ClimatePlugin {
    ...
    async fn execute(&self, params: serde_json::Value) -> GeoResult<serde_json::Value> {
        ...
        match command {
            "delta_downscale" => {
                let obs: [f64; 12] = serde_json::from_value(params["obs"].clone())
                    .map_err(GeoError::Serde)?;
                let hist: [f64; 12] = serde_json::from_value(params["hist"].clone())
                    .map_err(GeoError::Serde)?;
                let proj: [f64; 12] = serde_json::from_value(params["proj"].clone())
                    .map_err(GeoError::Serde)?;
            ...
            "quantile_mapping" => {
                let obs: Vec<f64> = serde_json::from_value(params["obs"].clone())
                    .map_err(GeoError::Serde)?;
                let hist: Vec<f64> = serde_json::from_value(params["hist"].clone())
                    .map_err(GeoError::Serde)?;
                let proj: Vec<f64> = serde_json::from_value(params["proj"].clone())
                    .map_err(GeoError::Serde)?;
                let result = gcm::quantile_mapping(&obs, &hist, &proj);
                serde_json::to_value(result).map_err(GeoError::Serde)
            }
            "idf_curve" => {
                let durations: Vec<f64> = serde_json::from_value(params["durations"].clone())
                    .map_err(GeoError::Serde)?;
                let p: idf::IdfParams = serde_json::from_value(params["params"].clone())
                    .map_err(GeoError::Serde)?;
            ...
            "spi" => {
                let precip: Vec<f64> = serde_json::from_value(params["precip"].clone())
                    .map_err(GeoError::Serde)?;
            ...
            "spei" => {
                let precip: Vec<f64> = serde_json::from_value(params["precip"].clone())
                    .map_err(GeoError::Serde)?;
                let temp: Vec<f64> = serde_json::from_value(params["temp"].clone())
                    .map_err(GeoError::Serde)?;
                let scale = params["scale_months"].as_u64().unwrap_or(3) as usize;
            ...
            "ordinary_kriging" => {
                let points: Vec<(f64, f64, f64)> = serde_json::from_value(params["points"].clone())
                    .map_err(GeoError::Serde)?;
                let bbox: geo_core::types::BBox = serde_json::from_value(params["bbox"].clone())
                    .map_err(GeoError::Serde)?;
                let cell_size = params["cell_size"].as_f64().unwrap_or(1.0);
                let variogram: kriging::VariogramParams =
                    serde_json::from_value(params["variogram"].clone())
                        .map_err(GeoError::Serde)?;
