...
impl CoastalPlugin {
    ...
    pub fn assess_shoreline(
        ...
        let n = ndvi_old.data.len().min(ndvi_new.data.len()).min(dem.data.len());
        for i in 0..n {
            let o = ndvi_old.data[i];
            let nv = ndvi_new.data[i];
            if o == ndvi_old.nodata || nv == ndvi_new.nodata { continue; }
            total += 1;

            // 陆地→水体: NDVI 显著下降
            if o > 0.2 && nv < 0.05 { eroded += 1; }

            // 海平面上升淹没: DEM < SLR
            let elev = dem.data[i];
            if elev != dem.nodata && elev < sea_level_rise_m { inundated += 1; }
        }

        let erosion_ratio = if total > 0 { eroded as f64 / total as f64 } else { 0.0 };
        let erosion_rate = if erosion_ratio > 0.0 { (erosion_ratio * 100.0) / years } else { 0.0 }; // %/yr → 近似 m/yr
        let inundated_ha = inundated as f64 * 0.01; // 10m像素→ha

        let (risk, summary) = if erosion_ratio > 0.1 || inundated_ha > 50.0 {
            ("🔴 高风险", format!("{aoi_name} 侵蚀显著 ({:.0}%)，淹没 {:.0} ha", erosion_ratio*100.0, inundated_ha))
        } else if erosion_ratio > 0.03 {
            ("🟡 中风险", format!("{aoi_name} 中度侵蚀 ({:.0}%)", erosion_ratio*100.0))
        } else {
            ("🟢 低风险", format!("{aoi_name} 岸线稳定"))
        };

        Ok(ShorelineReport {
            aoi_name: aoi_name.to_string(), bbox,
            baseline_year, assessment_year,
...
mod tests {
    use super::*;
    fn band(d: Vec<f64>) -> RasterBand { RasterBand::new("t", d.len(), 1, d, -999.0) }
