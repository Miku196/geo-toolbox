...
### Round 1：最小可行核心 + 一个完整案例（目标：1-2 天完成）
...
| Phase | cargo check | cargo test | cargo clippy | 额外检查 |
|-------|:-----------:|:----------:|:------------:|------|
| 1. Core 提纯 | ✅ | ✅ 51 tests | ✅ | 无破坏 |
| 2. Core 重组 | ✅ | ✅ ≥ 51 tests | ✅ | **每新 crate 独立可测；验证 core 层内部 DAG 无循环** |
| 3. Adapter 重组 | ✅ | ✅ | ✅ | 原有测试不变 |
| 4. Plugin 体系 | ✅ | ✅ | ✅ | **每插件独立可测；验证无跨插件 import** |
| 5. Registry | ✅ | ✅ ≥ 80 tests | ✅ | 含 MCP 集成测试 |
| 6. CLI 适配 | ✅ | ✅ ≥ 85 tests | ✅ | 最终验收 |
...
| CI/CD | `.github/workflows/ci.yml`：`cargo test --workspace` + `clippy` + `rustfmt` |
| Benchmark | criterion 基准测试套件，覆盖 CRS 变换、碳核算、NDVI |
